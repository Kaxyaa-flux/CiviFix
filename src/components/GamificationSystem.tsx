...
export default function GamificationSystem({ 
  ...
  const [rewards, setRewards] = useState<RewardItem[]>([
    {
      ...
      cost: 150,
      icon: <Bus className="w-5 h-5 text-[#7C3AED]" />,
  ...
  const badges: Badge[] = [
    {
      ...
      icon: <Compass className="w-5 h-5 text-[#7C3AED]" />,
      category: 'reporting',
      unlocked: true,
      progressCurrent: 1,
      progressTarget: 1,
  ...
  const getAvatarColor = (avatarId: string) => {
    ...
    return colors[avatarId] || 'bg-[#7C3AED] text-slate-900 dark:text-white';
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-white/10">
          <div className="space-y-1.5">
            ...
            <h1 className="text-3.5xl sm:text-4xl font-extrabold tracking-tight text-slate-950 dark:text-slate-900 dark:text-white flex flex-wrap items-center gap-3">
              <Award className="w-9 h-9 text-[#7C3AED] animate-pulse" />
              Civic <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-green-500 to-indigo-600">Rewards & Achievements</span>
            </h1>
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <button
              ...
              className={`px-5 py-3 rounded-2xl text-xs font-mono font-black flex items-center justify-center gap-2 cursor-pointer shadow-md transition-all duration-300 transform active:scale-98 border ${
                dailyClaimed 
                  ? 'bg-slate-100 dark:bg-[#16161D]/90 border-white/10 text-slate-400 cursor-not-allowed' 
                  : 'bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-slate-900 dark:text-white border-transparent'
              }`}
            >
              <Flame className={`w-4 h-4 ${dailyClaimed ? 'text-slate-400' : 'text-slate-900 dark:text-white animate-bounce'}`} />
        ...
        <AnimatePresence>
          {showPointPulse && (
            <motion.div
              ...
              className="fixed bottom-10 right-10 bg-gradient-to-r from-amber-500 to-orange-500 text-slate-900 dark:text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 z-50 border border-amber-400/20 font-sans"
            >
              <div className="bg-[#16161D]/90 backdrop-blur-md/20 p-2 rounded-xl">
                <Sparkles className="w-5 h-5 text-slate-900 dark:text-white animate-spin" />
              </div>
              <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="bg-[#16161D]/90 backdrop-blur-md dark:bg-[#16161D]/90 border border-white/10 rounded-3xl p-6 relative overflow-hidden shadow-xs">
              <div className="absolute top-0 right-0 w-32 h-32 bg-[#7C3AED]/5 rounded-bl-full pointer-events-none" />
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-extrabold text-2xl shadow-md ${
                    currentUser ? getAvatarColor(currentUser.avatar) : 'bg-gradient-to-tr from-cyan-500 to-blue-600 text-slate-900 dark:text-white'
                  }`}>
                    {currentUser ? currentUser.fullName.charAt(0).toUpperCase() : 'G'}
                  </div>
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <h2 className="text-lg font-black text-slate-950 dark:text-slate-900 dark:text-white">
                        {currentUser ? currentUser.fullName : 'Guest Explorer (Demo Profile)'}
                      </h2>
                      <span className="text-[10px] bg-[#7C3AED]/10 text-[#7C3AED] dark:text-blue-400 border border-white/10/15 px-2 py-0.5 rounded-full font-mono uppercase font-extrabold">
                        LEVEL {currentLevel}
                      </span>
                    </div>
                ...
                <div className="text-left sm:text-right">
                  ...
                  <p className="text-3xl font-black text-[#7C3AED] dark:text-blue-400 tracking-tight">{localPoints} PTS</p>
                </div>
              </div>
              ...
              <div className="space-y-2 mt-6">
                ...
                <div className="w-full bg-slate-100 dark:bg-[#0A0A0F] h-3 rounded-full overflow-hidden border border-slate-250 dark:border-white/5">
                  <motion.div 
                    initial={{ width: 0 }}
              ...
              {!currentUser && (
                <div className="bg-amber-500/5 border border-amber-500/15 p-3 rounded-2xl mt-5 flex items-center justify-between text-xs gap-3">
                  ...
                  <button 
                    ...
                    className="text-xs text-[#7C3AED] hover:text-[#7C3AED] font-bold shrink-0 cursor-pointer flex items-center gap-0.5 font-mono"
                  >
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 rounded-2xl border transition-all relative overflow-hidden flex items-start gap-3.5 ${
                        badge.unlocked 
                          ? 'bg-[#16161D]/90 backdrop-blur-md/50 dark:bg-[#0A0A0F]/20 border-white/10' 
                          : 'bg-slate-100/40 dark:bg-[#0A0A0F]/5 border-slate-200/50 dark:border-white/5/60 opacity-60'
                      }`}
                    >
                      ...
                      <div className={`w-11 h-11 rounded-xl shrink-0 flex items-center justify-center relative border shadow-xs ${
                        badge.unlocked 
                          ? 'bg-[#16161D]/90 backdrop-blur-md dark:bg-[#16161D]/90 border-indigo-500/30 text-indigo-500' 
                          : 'bg-slate-100 dark:bg-[#0A0A0F] border-white/10 text-slate-400'
                      }`}>
                        {badge.icon}
                        {badge.unlocked && (
                          <div className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-green-500 rounded-full flex items-center justify-center border border-white dark:border-slate-900">
                            <Check className="w-2.5 h-2.5 text-slate-900 dark:text-white" />
                          </div>
                        )}
                      </div>
                      ...
                      <div className="space-y-1 flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-1">
                          <h4 className="text-xs font-black truncate text-slate-950 dark:text-slate-900 dark:text-white">{badge.title}</h4>
                        ...
                        {!badge.unlocked && (
                          <div className="space-y-1 pt-1.5">
                            ...
                            <div className="w-full bg-slate-200 dark:bg-[#16161D]/90 h-1 rounded-full overflow-hidden">
                              <div 
            ...
            <div className="bg-gradient-to-r from-blue-900 to-indigo-900 text-slate-900 dark:text-white rounded-3xl p-6 relative overflow-hidden border border-blue-800/20 shadow-lg">
              ...
              <div className="space-y-4 relative z-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {INITIAL_CHAMPIONS.map((champ, idx) => (
                    <div 
                      key={idx}
                      className="bg-[#16161D]/90 backdrop-blur-md/5 border border-white/10 p-4 rounded-2xl space-y-3 hover:border-white/20 transition-all text-xs"
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="text-xs font-black text-slate-900 dark:text-white leading-tight">{champ.name}</h4>
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            {/* Live Leaderboard */}
            <div className="bg-[#16161D]/90 backdrop-blur-md dark:bg-[#16161D]/90 border border-white/10 rounded-3xl p-6 shadow-xs space-y-5">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-white/5">
                <h3 className="text-sm font-extrabold text-slate-950 dark:text-slate-900 dark:text-white font-mono uppercase tracking-wider flex items-center gap-1.5">
                  <Users className="w-4 h-4 text-[#7C3AED]" />
                  Live Hero Leaderboard
                </h3>

                <div className="flex rounded-lg bg-slate-100 dark:bg-[#0A0A0F] p-1 border border-slate-200 dark:border-white/5">
                  <button
                    ...
                    className={`px-2.5 py-1 text-[10px] font-mono font-bold rounded-md transition-all cursor-pointer ${
                      leaderboardTab === 'monthly'
                        ? 'bg-[#16161D]/90 backdrop-blur-md dark:bg-[#16161D]/90 text-[#7C3AED] dark:text-blue-400 shadow-xs'
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 rounded-2xl border transition-all flex items-center justify-between gap-3 ${
                        isCurrentUser 
                          ? 'bg-[#7C3AED]/10 border-white/10/55 shadow-xs' 
                          : 'bg-slate-50 dark:bg-transparent/30 border-slate-200 dark:border-white/5'
                      }`}
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="space-y-0.5 truncate">
                          <h4 className="text-xs font-extrabold text-slate-950 dark:text-slate-900 dark:text-white truncate flex items-center gap-1">
                            {item.name}
                            {isCurrentUser && (
                              <span className="text-[8px] bg-[#7C3AED] text-slate-900 dark:text-white font-mono uppercase px-1.5 py-0.2 rounded">YOU</span>
                            )}
                          </h4>
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-2.5">
                          <div className="w-9 h-9 rounded-xl bg-[#16161D]/90 backdrop-blur-md dark:bg-[#16161D]/90 border border-white/10 flex items-center justify-center shrink-0">
                            {reward.icon}
                          </div>
                          <div>
                            <span className="text-[8px] font-mono text-slate-400 font-bold uppercase">{reward.id}</span>
                            <h4 className="text-xs font-black text-slate-950 dark:text-slate-900 dark:text-white leading-tight">{reward.title}</h4>
                          </div>
                        </div>
                      ...
                      {/* Claim Action */}
                      <div className="flex items-center justify-between pt-2.5 border-t border-slate-100 dark:border-white/5/60 text-[10px] font-mono">
                        ...
                        {reward.isClaimed ? (
                          <span className="text-green-500 font-bold flex items-center gap-1 uppercase bg-green-500/10 border border-green-500/20 px-2 py-0.5 rounded-full">
                            <CheckCircle2 className="w-3 h-3 text-green-500" /> Redeemed
                          </span>
                        ) : (
                          <button
                            ...
                            className={`font-bold font-mono px-3.5 py-1.5 rounded-lg flex items-center gap-1 cursor-pointer transition-colors ${
                              canAfford 
                                ? 'bg-emerald-600 hover:bg-emerald-700 text-slate-900 dark:text-white' 
          ...
          {redeemedReward && (
            <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4 z-55">
              <motion.div 
                ...
                className="bg-[#16161D]/90 backdrop-blur-md dark:bg-[#16161D]/90 border border-white/10 rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-5 text-center relative"
              >
                ...
                <div className="w-16 h-16 rounded-2xl bg-green-500/10 border border-green-500/20 mx-auto flex items-center justify-center text-green-500">
                  <CheckCircle2 className="w-9 h-9 animate-bounce" />
                </div>

                <div className="space-y-2">
                  <span className="text-[10px] font-mono text-green-500 uppercase font-black tracking-widest">REDEMPTION SECURED</span>
                  <h3 className="text-xl font-black text-slate-950 dark:text-slate-900 dark:text-white">{redeemedReward.title}</h3>
                ...
                <div className="bg-slate-50 dark:bg-transparent p-4 rounded-2xl border border-dashed border-green-500/30 font-mono">
                  <p className="text-[10px] text-slate-400 uppercase tracking-wider mb-1">PROMOTION VOUCHER CODE</p>
                  <p className="text-lg font-black text-green-500 tracking-wider select-all">{redeemedReward.couponCode}</p>
                </div>

                <button
                  onClick={() => setRedeemedReward(null)}
                  className="w-full bg-slate-950 dark:bg-[#16161D]/90 backdrop-blur-md text-slate-900 dark:text-white dark:text-slate-950 py-3 rounded-xl font-black text-xs font-mono transition-colors cursor-pointer uppercase tracking-wider"
                >
                  Return to Dashboard
                </button>
              </motion.div>
